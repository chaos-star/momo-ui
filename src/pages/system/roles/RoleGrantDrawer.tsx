...
function collectMenuTreeKeys(nodes: PermissionNode[] = []) {
...
export default function RoleGrantDrawer({
  ...
  const menuKeySet = useMemo(() => collectMenuTreeKeys(menuTree), [menuTree]);

  const treeCheckedKeys = useMemo(
    () => checkedKeys.filter((k) => menuKeySet.has(k)),
    [checkedKeys, menuKeySet]
  );
  ...
  const handleMenuCheck = (keys: string[]) => {
    ...
    const next = new Set(checkedKeys.filter((k) => !menuKeySet.has(k)));
    keys.forEach((k) => next.add(k));
    const removed = [...prev].filter((k) => menuKeySet.has(k) && !next.has(k));
